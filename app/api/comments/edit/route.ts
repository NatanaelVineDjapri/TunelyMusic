...
export async function PUT(req: NextRequest) {
  try {
    // Ambil ID dan Comment baru dari body JSON
    const body = await req.json();
    const { id, comment } = body;

    if (!id || !comment) {
      return NextResponse.json({ message: "ID dan teks comment diperlukan" }, { status: 400 });
    }

    // Eksekusi Query Update
    const updateQuery = db.prepare("UPDATE comments SET comment = ? WHERE id = ?");
